...
def index(request, id):
    ...
    if request.method == 'POST':
        print(request.FILES['depth-image'], request.POST)
        ...
        # plt.barh(np.arange(len(hg_raw)), hg_raw)
        # plt.barh(np.arange(len(hg_smoothed)), hg_smoothed)
        ...
        body_length_smoothed = weight_utils.length_of_depth_vec(bl_smoothed)*39.37
